...
The ACP adapter wraps Kopi' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> Kopi `once`
- `allow_always` -> Kopi `always`
- reject options -> Kopi `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Kopi tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Kopi' runtime resolver:

- `acp_adapter/auth.py`
- `kopi_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Kopi provider/credentials.
