...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Kopi already has applies. The plugin never sees a
...
[`kopi-example-plugins`](https://github.com/XingBaoKu/kopi-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
kopi-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/XingBaoKu/kopi-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Kopi provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Kopi provider
        # (must be one Kopi already knows about — same names as
        ...
        # plugin sends — Kopi does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Kopi' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Kopi subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/XingBaoKu/kopi-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/XingBaoKu/kopi-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/XingBaoKu/kopi-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/XingBaoKu/kopi-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
