...
[PR #12550](https://github.com/XingBaoKu/kopi-agent/pull/12550) proposed a
...
(`kopi-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Kopi `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `kopi-acp`, `kopi-api-server`, core toolsets (gated on CDP reachability)
...
- `kopi_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
