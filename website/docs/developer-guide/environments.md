...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Kopi-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

KOPI O Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/XingBaoKu/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Kopi/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- KopiAgentBaseEnv
    KopiAgentBaseEnv <|-- TerminalTestEnv
    KopiAgentBaseEnv <|-- KopiSweEnv
    KopiAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The kopi-agent layer (`environments/kopi_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls kopi-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `KopiAgentLoop` and scores the result
...
Your environment inherits from `KopiAgentBaseEnv` and implements five methods:
...
`KopiAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as kopi-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any kopi-agent tool |
...
parser = get_parser("kopi")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `kopi`, `mistral`, `llama3_json`, `llama4_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1` (alias `deepseek_v31`), `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
Dataset: [XingBaoKu/terminal-bench-2](https://huggingface.co/datasets/XingBaoKu/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [XingBaoKu/openthoughts-tblite](https://huggingface.co/datasets/XingBaoKu/openthoughts-tblite).
...
pip install "kopi-agent[yc-bench]"
...
```bash
python environments/kopi_swe_env/kopi_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"kopi"`, `"qwen"`, `"mistral"`)
...
```python
from environments.kopi_base_env import KopiAgentBaseEnv, KopiAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(KopiAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(KopiAgentBaseEnv):
    name = "my-env"
...
### KopiAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which kopi toolsets to enable |
...
| `tool_call_parser` | `str` | `"kopi"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "XingBaoKu/terminal-bench-2"
  tokenizer_name: "XingBaoKu/Kopi-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/XingBaoKu/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "kopi-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "kopi-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── kopi_base_env.py          # Abstract base class (KopiAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (KopiAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── kopi_parser.py        # Kopi/ChatML <tool_call> format
...
├── kopi_swe_env/             # SWE-bench training environment
│
