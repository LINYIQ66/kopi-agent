...
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `KOPI_TUI=1`. |
...
| `kopi auth` | Manage credentials — add, list, remove, reset, set strategy. Handles OAuth flows for Codex/Nous/Anthropic. |
...
| `kopi plugins` | Manage KOPI O Agent plugins (install, enable, disable, remove). |
...
| `kopi update` | Pull latest code and reinstall dependencies. `--check` prints commit diff without pulling; `--backup` takes a pre-pull `KOPI_HOME` snapshot. |
...
| `--provider <provider>` | `KOPI_INFERENCE_PROVIDER` | Override the provider for this run |
...
:::tip WSL users
Use `kopi gateway run` instead of `kopi gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s kopi 'kopi gateway run'`. See [WSL FAQ](/docs/reference/faq#wsl-gateway-keeps-disconnecting-or-kopi-gateway-start-fails) for details.
:::

## `kopi setup`

```bash
kopi setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure]
```
...
For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/kopi-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/docs/user-guide/features/kanban).
...
The report includes system info (OS, Python version, Kopi version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Create a zip archive of your Kopi configuration, skills, sessions, and data. The backup excludes the kopi-agent codebase itself.
...
- The `kopi-agent` code itself (this is a user-data backup, not a repo snapshot).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Kopi Plugin](../guides/build-a-kopi-plugin.md).
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. Requires `pip install kopi-agent[web]` (FastAPI + Uvicorn). See [Web Dashboard](/docs/user-guide/features/web-dashboard) for full documentation.
...
Pulls the latest `kopi-agent` code and reinstalls dependencies in your venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install.
...
| `--backup` | Create a labeled pre-update snapshot of `KOPI_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `update.backup: true` in `config.yaml`. |
...
| `kopi uninstall [--full] [--yes]` | Remove Kopi, optionally deleting all config/data. |
