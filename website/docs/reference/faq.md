...
description: "Frequently asked questions and solutions to common issues with KOPI O Agent"
---
...
KOPI O Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Xing Bao Ku PTE LTD's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `kopi login anthropic`, OpenRouter, or any compatible proxy)
...
**Not natively.** KOPI O Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Kopi from inside it. The standard install command works perfectly in WSL2:
...
Yes — Kopi now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). KOPI O Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.kopi/`.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Kopi — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
...
KOPI O Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same KOPI O Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
pip install "kopi-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
- [Use MCP with Kopi](/docs/guides/use-mcp-with-kopi)
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `kopi profile create newname --clone-all` to copy everything from the current profile.
...
1. Install KOPI O Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/XingBaoKu/kopi-agent/main/scripts/install.sh | bash
   ```
...
```bash
rsync -av --exclude='kopi-agent' ~/.kopi/ newmachine:~/.kopi/
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/XingBaoKu/kopi-agent/issues)
2. **Ask the community:** [Xing Bao Ku PTE LTD Discord](https://discord.gg/xingbaoku)
