...
description: "Install and deploy KOPI O Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

KOPI O Agent ships a Nix flake with three levels of integration:
...
nix run github:XingBaoKu/kopi-agent -- setup
nix run github:XingBaoKu/kopi-agent -- chat

# Or install persistently
nix profile install github:XingBaoKu/kopi-agent
kopi setup
kopi chat
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    kopi-agent.url = "github:XingBaoKu/kopi-agent";
  };
...
For optional extras already declared in kopi-agent's `pyproject.toml` (e.g., memory providers like `hindsight` or `honcho`), use `extraDependencyGroups` to include them in the sealed venv at build time:
...
```nix
{
  inputs.kopi-agent.url = "github:XingBaoKu/kopi-agent";
