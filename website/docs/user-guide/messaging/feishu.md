...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up KOPI O Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

KOPI O Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Kopi opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Kopi responds to every message. |
| Group chats | Kopi responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Kopi will automatically create a bot application with the correct permissions and save the credentials.
...
Use WebSocket mode when Kopi runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Kopi behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Kopi starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Kopi responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Kopi responds to @mentions from any user in any group. |
| `allowlist` | Kopi only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Kopi ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Kopi read all group traffic without requiring an @mention:
...
Kopi auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Kopi ignores messages sent by other bots. Enable bot-to-bot messaging when you want Kopi to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Kopi. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `kopi update --gateway` needs confirmation, the adapter records the selected answer in Kopi's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Kopi reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
| `Another local Kopi gateway is already using this Feishu app_id` | Only one Kopi instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Kopi can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
