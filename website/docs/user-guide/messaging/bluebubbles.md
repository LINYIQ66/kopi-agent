...
Connect Kopi to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Kopi and the BlueBubbles server
...
When someone messages your iMessage, Kopi automatically sends them a pairing code. Approve it with:
```bash
...
Kopi will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Kopi
Kopi → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Kopi sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Kopi resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
