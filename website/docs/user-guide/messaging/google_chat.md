...
sidebar_position: 12
title: "Google Chat"
description: "Set up KOPI O Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect KOPI O Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Kopi
...
download the file. Save it somewhere only Kopi can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Kopi calls
...
- **App name**: whatever you want users to see ("Kopi" is reasonable).
...
`ADDED_TO_SPACE` event that Kopi uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Kopi
...
Install the dependencies the Google Chat adapter needs (no Kopi extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Kopi is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Kopi detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Kopi session.
...
3. Download the JSON. Move it onto the host that runs Kopi.
4. On the host, register the client with Kopi:
...
That writes `~/.kopi/google_chat_user_client_secret.json`. This is shared
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Kopi isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Kopi will only attach the SA bearer
