...
sidebar_position: 10
title: "DingTalk"
description: "Set up KOPI O Agent as a DingTalk chatbot"
---

# DingTalk Setup

KOPI O Agent integrates with DingTalk (钉钉) as a chatbot, letting you chat with your AI assistant through direct messages or group chats. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how Kopi behaves once it's in your DingTalk workspace.

## How Kopi Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | Kopi responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | Kopi responds when you `@mention` it. Without a mention, Kopi ignores the message. |
| **Shared groups with multiple users** | By default, Kopi isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
```bash
pip install "kopi-agent[dingtalk]"
```
...
KOPI O Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
Kopi can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
Kopi automatically adds emoji reactions to your messages to show processing status:
