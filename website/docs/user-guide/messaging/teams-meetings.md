...
Use the Teams meeting pipeline when you want Kopi to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Use this when you want Kopi to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start Kopi normally after updating config:

```bash
kopi gateway run
```

Or, if you run Kopi in Docker, start the gateway the same way you already do for your deployment.
