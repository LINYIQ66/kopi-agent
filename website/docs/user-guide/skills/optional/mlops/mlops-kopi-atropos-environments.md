---
title: "Kopi Atropos Environments — Build, test, and debug KOPI O Agent RL environments for Atropos training"
sidebar_label: "Kopi Atropos Environments"
description: "Build, test, and debug KOPI O Agent RL environments for Atropos training"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Kopi Atropos Environments

Build, test, and debug KOPI O Agent RL environments for Atropos training. Covers the KopiAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the kopi-agent repo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kopi skills install official/mlops/kopi-atropos-environments` |
| Path | `optional-skills/mlops/kopi-atropos-environments` |
| Version | `1.1.0` |
| Author | KOPI O Agent |
| License | MIT |
...
# KOPI O Agent Atropos Environments

Guide for building RL environments in the kopi-agent repo that integrate with the Atropos training framework.
...
Atropos BaseEnv (atroposlib/envs/base.py)
    └── KopiAgentBaseEnv (environments/kopi_base_env.py)
...
Kopi environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/kopi_base_env.py` | Base class with agent loop + tool resolution |
| `environments/agent_loop.py` | `KopiAgentLoop` + `AgentResult` dataclass |
| `environments/tool_context.py` | `ToolContext` for reward verification |
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (kopi, mistral, etc.) |
...
The whole point of kopi-agent environments is agentic evaluation:

```python
async def evaluate(self, *args, **kwargs) -> None:
    import time, uuid
    from environments.agent_loop import KopiAgentLoop
...
2. **evaluate() must use KopiAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of kopi-agent benchmarks is agentic evaluation with tool use.
