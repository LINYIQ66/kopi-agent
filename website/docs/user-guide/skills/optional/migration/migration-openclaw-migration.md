---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into KOPI O Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into KOPI O Agent"
---
...
Migrate a user's OpenClaw customization footprint into KOPI O Agent. Imports Kopi-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | KOPI O Agent (Xing Bao Ku PTE LTD) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Kopi`, `Memory`, `Persona`, `Import` |
| Related skills | [`kopi-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kopi-agent) |
...
# OpenClaw -> Kopi Migration

Use this skill when a user wants to move their OpenClaw setup into KOPI O Agent with minimal manual cleanup.
...
- import `SOUL.md` into the Kopi home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Kopi memory entries
- merge OpenClaw command approval patterns into Kopi `command_allowlist`
- migrate Kopi-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.kopi/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Kopi workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.kopi/tts/`
- archive non-secret docs that do not have a direct Kopi destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Kopi-compatible secrets, currently:
...
Kopi CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Kopi. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Kopi workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Kopi skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Kopi unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Kopi targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Kopi destination. Unsupported auth blobs must still be reported as skipped.
...
- Kopi persona state imported
- Kopi memory files populated with converted OpenClaw knowledge
