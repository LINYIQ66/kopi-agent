...
title: "Git Worktrees"
description: "Run multiple Kopi agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

KOPI O Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Kopi so each session has a clean, isolated working directory.

## Why Use Worktrees with Kopi?

Kopi treats the **current working directory** as the project root:
...
Now you can `cd` into the new worktree and run Kopi there:

```bash
cd ../repo-feature

# Start Kopi in the worktree
kopi
```
...
- Kopi checkpoint data under `~/.kopi/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Kopi experiment**
...
- **Avoid running Kopi from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `kopi -w` (Automatic Worktree Mode)

Kopi has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Use **git worktrees** to give each Kopi session its own clean checkout.
