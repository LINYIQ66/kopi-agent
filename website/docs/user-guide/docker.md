...
sidebar_position: 7
title: "Docker"
description: "Running KOPI O Agent in Docker and using Docker as a terminal backend"
---

# KOPI O Agent — Docker

There are two distinct ways Docker intersects with KOPI O Agent:
...
If this is your first time running KOPI O Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.kopi
docker run -it --rm \
  -v ~/.kopi:/opt/data \
  xingbaoku/kopi-agent setup
```
...
docker run -d \
  ...
  -v ~/.kopi:/opt/data \
  -p 8642:8642 \
  xingbaoku/kopi-agent gateway run
```
...
The built-in web dashboard runs as an optional side-process inside the same container as the gateway. Set `KOPI_DASHBOARD=1` and expose port `9119` alongside the gateway's `8642`:
...
The entrypoint starts `kopi dashboard` in the background (running as the non-root `kopi` user) before `exec`-ing the main command. Dashboard output is prefixed with `[dashboard]` in `docker logs` so it's easy to separate from gateway logs.

| Environment variable | Description | Default |
|---------------------|-------------|---------|
| `KOPI_DASHBOARD` | Set to `1` (or `true` / `yes`) to launch the dashboard alongside the main command | *(unset — dashboard not started)* |
...
| `KOPI_DASHBOARD_TUI` | Set to `1` to expose the in-browser Chat tab (embedded `kopi --tui` via PTY/WebSocket) | *(unset)* |

The default `KOPI_DASHBOARD_HOST=0.0.0.0` is required for the host to reach the dashboard through the published port; the entrypoint automatically passes `--insecure` to `kopi dashboard` in that case. Override to `127.0.0.1` if you want to restrict the dashboard to in-container access only (e.g. behind a reverse proxy in a sidecar).
...
Kopi supports [multiple profiles](../reference/profile-commands.md) — separate `~/.kopi/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using Kopi' built-in multi-profile feature is not recommended.**
...
- **Independent lifecycle** — upgrade, restart, pause, or roll back each agent separately (`docker restart kopi-work` leaves `kopi-personal` untouched).
...
services:
  kopi-work:
    image: xingbaoku/kopi-agent:latest
...
- Python 3 with all Kopi dependencies (`uv pip install -e ".[all]"`)
...
- Optionally launches `kopi dashboard` as a background side-process when `KOPI_DASHBOARD=1` (see [Running the dashboard](#running-the-dashboard))
- Then runs `kopi` with whatever arguments you pass

:::warning
Do not override the image entrypoint unless you keep `/opt/kopi/docker/entrypoint.sh` in the command chain. The entrypoint drops root privileges to the `kopi` user before gateway state files are created. Starting `kopi gateway run` as root inside the official image is refused by default because it can leave root-owned files in `/opt/data` and break later dashboard or gateway starts. Set `KOPI_ALLOW_ROOT_GATEWAY=1` only when you intentionally accept that risk.
:::
...
```sh
docker pull xingbaoku/kopi-agent:latest
docker rm -f kopi
docker run -d \
  --name kopi \
  --restart unless-stopped \
...
The container's entrypoint drops privileges to the non-root `kopi` user (UID 10000) via `gosu`. If your host `~/.kopi/` is owned by a different UID, set `KOPI_UID`/`KOPI_GID` to match your host user, or ensure the data directory is writable:
...
docker run -it --rm xingbaoku/kopi-agent:latest version     # Verify version
