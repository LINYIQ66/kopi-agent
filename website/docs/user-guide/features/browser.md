...
KOPI O Agent includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.happysocial.xyz) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. Run `kopi model` or `kopi tools` to enable it.
:::
...
When a cloud provider is configured, Kopi auto-spawns a **local Chromium sidecar**
...
Then fully restart Kopi so the new config is picked up.

:::warning Nested path matters
Kopi reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Kopi ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Kopi silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Kopi profile, so different Kopi profiles get different browser profiles (profile isolation).

##### What Kopi does not do
- It does not force persistence on the Camofox server. Kopi only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Kopi cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Kopi and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Kopi after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Kopi derives the stable `userId` from the profile-scoped directory `~/.kopi/browser_auth/camofox/` (or the equivalent under `$KOPI_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Kopi profile's state directory.

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Kopi automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chrome via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Kopi browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Kopi from the terminal (`kopi` or `kopi chat`) and issue `/browser connect` there.
:::
...
If Chrome isn't already running with remote debugging, Kopi will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
Then launch the Kopi CLI and run `/browser connect`.
...
If Kopi runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Kopi itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Kopi talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Kopi MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Kopi can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
```
Navigate to https://github.com/XingBaoKu
```
...
:::note
If paid features aren't available on your plan, Kopi automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
