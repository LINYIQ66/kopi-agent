...
Schedule tasks to run automatically with natural language or cron expressions. Kopi exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Kopi itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::warning
Cron-run sessions cannot recursively create more cron jobs. Kopi disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
Kopi will use the unified `cronjob` tool internally.
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.kopi/cron/output/`), but no message is sent to the delivery target.
...
Or set the `KOPI_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The `cronjob` tool's schema exposes `no_agent` to Kopi directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Kopi will write the check script to `~/.kopi/scripts/` via `write_file`, then call:
...
- When Job 2 fires, Kopi reads Job 1's most recent output from `~/.kopi/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Kopi skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `kopi tools` cron-platform config wins; otherwise Kopi falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Kopi should even invoke the agent. Emit a final stdout line of the form:
...
Jobs may store `model` and `provider` as `null`. When those fields are omitted, Kopi resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
