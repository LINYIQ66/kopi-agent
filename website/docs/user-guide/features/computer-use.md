...
KOPI O Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `kopi computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `kopi tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Kopi refreshes the binary in two
places so you don't get stuck on a stale release:

- **`kopi update`** — when you update Kopi itself, if `cua-driver` is
...
- **`kopi computer-use install --upgrade`** — manual force-refresh.
...
`kopi computer-use status` shows the installed version next to the
binary path.
...
Kopi applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.kopi/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Kopi applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `KOPI_CUA_DRIVER_VERSION`
...
KOPI_CUA_DRIVER_VERSION=0.5.0    # optional pin
```
...
`kopi computer-use install` to fetch the cua-driver binary, or run
`kopi tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/XingBaoKu/kopi-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
