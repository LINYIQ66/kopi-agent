...
You can also point Kopi at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Kopi. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `kopi skills search <query> --source skills-sh`. Kopi resolves alias-style skills when the skills.sh slug differs from the repo folder. |
