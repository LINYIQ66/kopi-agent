...
The web dashboard is a browser-based UI for managing your KOPI O Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `kopi --tui` via PTY/WebSocket). Alternatively set `KOPI_DASHBOARD_TUI=1`. |
...
The default `kopi-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'kopi-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install kopi-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`pip install 'kopi-agent[web,pty]'`, or `[all]` covers both)
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.kopi/skills/` and grouped by category.
...
| Theme | Character |
|-------|-----------|
| **Kopi Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Kopi Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
