...
description: "Build themes and plugins for the Kopi web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Kopi web dashboard (`kopi dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **Kopi Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Kopi Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Kopi Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Kopi plugins in `~/.kopi/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
#### Calling built-in Kopi endpoints
...
| `header-left` | Before the Kopi brand in the top bar. |
...
A reference plugin (`example-dashboard` in [`kopi-example-plugins`](https://github.com/XingBaoKu/kopi-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
#### Accessing Kopi internals

Backend routes run inside the dashboard process, so they can import from the kopi-agent codebase directly:
...
The [`strike-freedom-cockpit`](https://github.com/XingBaoKu/kopi-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `kopi-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/XingBaoKu/kopi-example-plugins.git
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Kopi Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
