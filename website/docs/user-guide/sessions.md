...
<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Kopi session." />
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`kopi insights`](/docs/reference/cli-commands#kopi-insights).
...
| Gateway index | `~/.kopi/sessions/sessions.json` | Maps session keys to active session IDs |
