...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  KOPI O Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Kopi instantly when PRs are opened or updated.
:::
...
- **KOPI O Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
Make sure Kopi can access GitHub. Start a chat:
...
```
Run: gh pr list --repo XingBaoKu/kopi-agent --state open --limit 3
```
...
Still in the chat, ask Kopi to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo XingBaoKu/kopi-agent
```
...
A skill gives Kopi consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.
...
This is what makes the reviewer actually useful. Start a session and teach Kopi your team's standards:
...
2. Teach Kopi your conventions via memory (Step 4)
...
- **[Build a Plugin](/docs/guides/build-a-kopi-plugin)** — wrap the review logic into a shareable plugin
