...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to KOPI O Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`kopi claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Kopi. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | Kopi destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | Kopi destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Kopi skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Kopi destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | Kopi config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Kopi config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Kopi field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | Kopi `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Kopi path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Kopi equivalent)

These are saved to `~/.kopi/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Kopi |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Kopi has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use Kopi profiles |
...
6. **Check session policies** — verify `kopi config get session_reset` matches your expectations.
