---
sidebar_position: 6
title: "Use MCP with Kopi"
description: "A practical guide to connecting MCP servers to KOPI O Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Kopi

This guide shows how to actually use MCP with KOPI O Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Kopi tool
- you want Kopi to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Kopi to internal APIs, databases, or company systems without modifying Kopi core

Do not use MCP when:
- a built-in Kopi tool already solves the job well
...
- Kopi remains the agent
- MCP servers contribute tools
- Kopi discovers those tools at startup or reload time
...
If you installed Kopi with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Kopi banner/status should show MCP integration when configured
- ask Kopi what tools it has available
...
## WSL2: bridge Kopi in WSL to Windows Chrome

This is the practical setup when:

- Kopi runs inside WSL2
...
In this setup, Kopi does **not** connect to Chrome directly. Instead:

- Kopi runs in WSL
- Kopi starts a local stdio MCP server
...
```text
Kopi (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Kopi stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Kopi core browser transport
...
Then start a fresh Kopi session or run:
...
Once loaded, Kopi can use the MCP-prefixed browser tools directly. For example:
...
If Kopi runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Kopi from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Kopi from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Kopi:
...
So Kopi will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Kopi to reason over a bounded workspace.
...
Now Kopi can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Kopi core.
...
Because Kopi now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
