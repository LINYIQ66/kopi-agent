---
sidebar_position: 7
title: "Use SOUL.md with Kopi"
description: "How to use SOUL.md to shape KOPI O Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Kopi

`SOUL.md` is the **primary identity** for your Kopi instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Kopi to feel like the same assistant every time you talk to it — or if you want to replace the Kopi persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Kopi should be
- what Kopi should avoid stylistically
- how Kopi should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Kopi is and how Kopi speaks
...
Kopi now uses only the global SOUL file for the current instance:

```text
~/.kopi/SOUL.md
```

If you run Kopi with a custom home directory, it becomes:
...
Kopi automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Kopi does not overwrite it
- if the file exists but is empty, Kopi adds nothing from it to the prompt

## How Kopi uses it

When Kopi starts a session, it reads `SOUL.md` from `KOPI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Kopi feels.
...
Kopi already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Kopi should sound.

# Avoid
What Kopi should not do.

# Defaults
How Kopi should behave when ambiguity appears.
```
...
Then restart Kopi or start a new session.
...
### I edited SOUL.md but Kopi still sounds the same
...
### Kopi is ignoring parts of my SOUL.md
