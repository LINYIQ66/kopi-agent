---
sidebar_position: 8
title: "Use Voice Mode with Kopi"
description: "A practical guide to setting up and using Kopi voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Kopi
...
- you want Kopi sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Kopi.
...
## Step 1: make sure normal Kopi works first
...
```bash
pip install "kopi-agent[tts-premium]"
```
...
Kopi supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Kopi checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Kopi transcribes and responds
...
- asking Kopi to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Kopi loop.

## Tuning CLI behavior

### Silence threshold

If Kopi starts/stops too aggressively, tune:
...
Kopi stays a normal chat bot, but can speak replies.
...
- you want Kopi to function like a portable research or ops assistant
...
Kopi joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Kopi detects speech boundaries
- transcripts are posted in the associated text channel
- Kopi responds in text and audio
...
1. get text Kopi working
2. install `kopi-agent[voice]`
