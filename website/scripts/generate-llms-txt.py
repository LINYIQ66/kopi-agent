#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Kopi docs site.
...
Both publish at:
  https://kopi-agent.happysocial.xyz/docs/llms.txt
  https://kopi-agent.happysocial.xyz/docs/llms-full.txt
...
SITE_BASE = "https://kopi-agent.happysocial.xyz/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Kopi as a Python Library", None),
        ("guides/use-mcp-with-kopi", "Use MCP with Kopi", None),
        ("guides/use-voice-mode-with-kopi", "Use Voice Mode with Kopi", None),
        ("guides/use-soul-with-kopi", "Use SOUL.md with Kopi", None),
        ("guides/build-a-kopi-plugin", "Build a Kopi Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Kopi"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# KOPI O Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Xing Bao Ku PTE LTD. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/XingBaoKu/"
        "kopi-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/XingBaoKu/kopi-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# KOPI O Agent — Full Documentation\n",
        (
            "This file is the entire KOPI O Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Kopi, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://kopi-agent.happysocial.xyz/docs\n",
        "Short index: https://kopi-agent.happysocial.xyz/docs/llms.txt\n",
        "\n---\n\n",
    ]
