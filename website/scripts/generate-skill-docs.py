...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/XingBaoKu/kopi-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with KOPI O Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Kopi ships with a large built-in skill library copied into `~/.kopi/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Kopi also syncs bundled skills on `kopi update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.kopi/skills/` tree, it is still shipped with Kopi; restore it with `kopi skills reset <name> --restore`.",
        "",
