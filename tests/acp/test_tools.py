...
class TestBuildToolTitle:
    ...
    def test_skill_manage_title_includes_action_and_target(self):
        title = build_tool_title(
            "skill_manage",
            {"action": "patch", "name": "kopi-agent-operations", "file_path": "references/acp.md"},
        )
        assert title == "skill patch: kopi-agent-operations/references/acp.md"
...
class TestBuildToolStart:
    ...
    def test_build_tool_start_for_skill_manage_patch_shows_diff(self):
        ...
        assert result.kind == "edit"
        assert result.title == "skill patch: kopi-agent-operations/references/acp.md"
        assert isinstance(result.content[0], FileEditToolCallContent)
        assert result.content[0].path == "skills/kopi-agent-operations/references/acp.md"
...
class TestBuildToolComplete:
    ...
    def test_build_tool_complete_for_skill_manage_summarizes_without_raw_json(self):
        result = build_tool_complete(
            "tc-skill-manage",
            "skill_manage",
            '{"success":true,"message":"Patched references/kopi-acp-zed-rendering.md in skill \'kopi-agent-operations\' (1 replacement)."}',
            function_args={
                "action": "patch",
                "name": "kopi-agent-operations",
        ...
        assert "`kopi-agent-operations`" in text
    ...
    def test_build_tool_complete_for_write_file_uses_snapshot_diff(self, tmp_path):
        ...
        assert diff_item.new_text == "hello from kopi"
