...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Kopi is unconfigured."""
        from kopi_cli import config as config_module
    ...
    def test_claude_code_creds_counted_when_kopi_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Kopi has been explicitly configured."""
        import yaml
        from kopi_cli import config as config_module
        kopi_home = tmp_path / ".kopi"
        kopi_home.mkdir()
