...
def test_offer_launch_chat_relaunches_via_bin(monkeypatch):
    ...
    monkeypatch.setattr(relaunch_mod, "resolve_kopi_bin", lambda: "/usr/local/bin/kopi")
    ...
    assert exec_calls == [("/usr/local/bin/kopi", ["/usr/local/bin/kopi", "chat"])]
...
def test_offer_launch_chat_falls_back_to_module(monkeypatch):
    ...
    assert exec_calls == [(sys.executable, [sys.executable, "-m", "kopi_cli.main", "chat"])]
