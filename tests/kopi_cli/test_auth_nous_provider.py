...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kopi_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        fake_auth_state = {
            ...
            "inference_base_url": "https://inference-api.happysocial.xyz",
            "portal_base_url": "https://portal.happysocial.xyz",
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Kopi is not logged into Nous Portal").
    ...
    """
    from kopi_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "Kopi is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Kopi must surface an
    ...
    re-authenticate" string led users to report this as a Kopi persistence
