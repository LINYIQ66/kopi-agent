...
Covers:
 - Normal path (tokens already in Kopi auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Kopi
...
def kopi_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Kopi auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(kopi_auth_only_env):
    """openai-codex appears when tokens are already in Kopi auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Kopi
    auth store."""
    kopi_home = tmp_path / ".kopi"
    kopi_home.mkdir()
