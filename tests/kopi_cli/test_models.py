...
class TestFetchOpenRouterModels:
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        kopi-agent is tool-calling-first — surfacing a non-tool model leads to
...
class TestCheckNousFreeTierCache:
    ...
    @patch("kopi_cli.models.fetch_nous_account_tier")
    @patch("kopi_cli.models.is_nous_free_tier", return_value=True)
    def test_result_is_cached(self, mock_is_free, mock_fetch):
        ...
        with patch("kopi_cli.auth.get_provider_auth_state", return_value={"access_token": "tok"}), \
             patch("kopi_cli.auth.resolve_nous_runtime_credentials"):
    ...
    @patch("kopi_cli.models.fetch_nous_account_tier")
    @patch("kopi_cli.models.is_nous_free_tier", return_value=False)
...
class TestNousRecommendedModels:
    ...
    def test_fetch_cache_is_keyed_per_portal(self):
        ...
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            ...
            fetch_nous_recommended_models("https://portal.staging-happysocial.xyz")
