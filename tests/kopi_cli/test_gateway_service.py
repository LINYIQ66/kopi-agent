...
class TestKopiHomeForTargetUser:
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".kopi" / "kopi-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".kopi" / "kopi-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".kopi" / "kopi-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".kopi" / "kopi-agent"
        ...
        assert "WorkingDirectory=/home/alice/.kopi/kopi-agent" in unit
...
class TestLegacyKopiUnitDetection:
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = f"kopi.service" if i == 0 else f"kopi.service"  # same name
            # Test each variant fresh
            (user_dir / "kopi.service").write_text(
                f"[Unit]\nDescription=Old Kopi\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_kopi_units()
...
class TestRemoveLegacyKopiUnits:
