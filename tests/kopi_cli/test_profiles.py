...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes kopi-agent, .worktrees,
        ...
        (default_home / "kopi-agent" / ".git").mkdir(parents=True)
        (default_home / "kopi-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "kopi-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "kopi-agent").exists()
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert cfg["hosts"]["kopi.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["kopi.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        assert "kopi.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["kopi.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["kopi.heimdall"]["workspace"] == "kopi"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        assert cfg["hosts"]["kopi.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("kopi-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/kopi-agent", "default/.worktrees", "default/profiles",
...
class TestCompletion:
    ...
    def test_bash_completion_has_kopi_profiles_function(self):
        script = generate_bash_completion()
        assert "_kopi_profiles" in script

    def test_zsh_completion_has_kopi_function(self):
        script = generate_zsh_completion()
        assert "_kopi" in script
