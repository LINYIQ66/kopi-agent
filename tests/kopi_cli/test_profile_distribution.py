...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            ...
            with pytest.raises(DistributionError, match="requires Kopi"):
                check_kopi_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Kopi distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_kopi_requires(self, profile_env, monkeypatch):
        # Pin current Kopi version to something well below the requirement
        import kopi_cli
