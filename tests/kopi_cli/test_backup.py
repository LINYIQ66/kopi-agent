...
def _make_kopi_tree(root: Path) -> None:
    ...
    # kopi-agent repo (should be EXCLUDED)
    (root / "kopi-agent").mkdir(exist_ok=True)
    (root / "kopi-agent" / "run_agent.py").write_text("# big file\n")
    (root / "kopi-agent" / ".git").mkdir()
    (root / "kopi-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_kopi_agent(self):
        ...
        assert _should_exclude(Path("kopi-agent/run_agent.py"))
        assert _should_exclude(Path("kopi-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_kopi_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include kopi-agent/ directory."""
        kopi_home = tmp_path / ".kopi"
        kopi_home.mkdir()
        _make_kopi_tree(kopi_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "kopi-agent" in n]
            assert agent_files == [], f"kopi-agent files leaked into backup: {agent_files}"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # kopi-agent should NOT be present
        assert not (dst_home / "kopi-agent").exists()
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, kopi_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # kopi-agent repo excluded
        assert not any(n.startswith("kopi-agent/") for n in names)
