...
class TestKopiConstantsFallback:
    """Tests for _kopi_home.py fallback when kopi_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_kopi_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _kopi_home.py with kopi_constants blocked."""
        monkeypatch.setitem(sys.modules, "kopi_constants", None)
        spec = importlib.util.spec_from_file_location("_kopi_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_kopi_home_env_var(self, monkeypatch, tmp_path):
        """When kopi_constants is missing, KOPI_HOME comes from env var."""
        monkeypatch.setenv("KOPI_HOME", str(tmp_path / "custom-kopi"))
        module = self._load_helper(monkeypatch)
        assert module.get_kopi_home() == tmp_path / "custom-kopi"

    def test_fallback_defaults_to_dot_kopi(self, monkeypatch):
        """When kopi_constants is missing and KOPI_HOME unset, default to ~/.kopi."""
        monkeypatch.delenv("KOPI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_kopi_home() == Path.home() / ".kopi"

    def test_fallback_ignores_empty_kopi_home(self, monkeypatch):
        """Empty/whitespace KOPI_HOME is treated as unset."""
        monkeypatch.setenv("KOPI_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_kopi_home() == Path.home() / ".kopi"

    def test_fallback_display_kopi_home_shortens_path(self, monkeypatch):
        """Fallback display_kopi_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("KOPI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_kopi_home() == "~/.kopi"

    def test_fallback_display_kopi_home_profile_path(self, monkeypatch):
        """Fallback display_kopi_home() handles profile paths under ~/."""
        monkeypatch.setenv("KOPI_HOME", str(Path.home() / ".kopi/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_kopi_home() == "~/.kopi/profiles/coder"

    def test_fallback_display_kopi_home_custom_path(self, monkeypatch):
        """Fallback display_kopi_home() returns full path for non-home locations."""
        monkeypatch.setenv("KOPI_HOME", "/opt/kopi-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_kopi_home() == "/opt/kopi-custom"

    def test_delegates_to_kopi_constants_when_available(self):
        """When kopi_constants IS importable, _kopi_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_kopi_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import kopi_constants
        assert module.get_kopi_home is kopi_constants.get_kopi_home
        assert module.display_kopi_home is kopi_constants.display_kopi_home
