...
"""
    ...
    assert "Tyler Williams > Active Projects: KOPI O Agent" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Kopi``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Kopi prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Kopi to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Kopi config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Kopi uses tools well"
    # All-lowercase matches → lowercase ``kopi``; this preserves the
    # real filesystem path ``~/.kopi`` (Kopi home) when rebranding
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Kopi remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "kopi prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Kopi was configured for Spanish"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Kopi said to ask Kopi about Kopi settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.kopi`` (the real Kopi home), not the broken
    ``.Kopi``.
    ...
    rewritten to ``~/.Kopi/config.yaml`` — a path that doesn't exist —
