...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # whose tools live in kopi-cli's universe (see toolsets.py).
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("kopi_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, KOPI_INFERENCE_PROVIDER must
    ...
    assert os.environ["KOPI_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set KOPI_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. KOPI_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["KOPI_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["KOPI_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["KOPI_TUI_PROVIDER"] == "anthropic"
        assert os.environ["KOPI_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["KOPI_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors KopiCLI._manual_compress's
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "kopi_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
