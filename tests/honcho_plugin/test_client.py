...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kopi-agent"
        ):
            result = config.resolve_session_name("/home/user/kopi-agent/subdir")
        assert result == "kopi-agent"
...
class TestResolveConfigPath:
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        kopi_home = tmp_path / "kopi"
        kopi_home.mkdir()
        # No honcho.json in KOPI_HOME — also isolate ~/.kopi so
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kopi.coder")
        assert config.host == "kopi.coder"
        assert config.workspace_id == "kopi"  # shared workspace
        assert config.ai_peer == "kopi.coder"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="kopi.coder", config_path=config_file,
        )
        assert config.host == "kopi.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="kopi.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "kopi.dreamer"
