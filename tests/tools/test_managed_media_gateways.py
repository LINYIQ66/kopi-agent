...
def _enable_managed_nous_tools(monkeypatch):
    ...
    monkeypatch.setattr("kopi_cli.auth.get_nous_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr("kopi_cli.models.check_nous_free_tier", lambda: False)
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "happysocial.xyz")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "nous-token")
    ...
    assert captured["base_url"] == "https://openai-audio-gateway.happysocial.xyz/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.happysocial.xyz/v1"
