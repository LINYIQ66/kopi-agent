...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.kopi/config.yaml``. Kopi
...
# custom voice-cloning scripts, etc.) into Kopi without any Python code
...
#           output_format: wav
#
# Kopi writes the input text to a temp UTF-8 file, runs the command with
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Kopi caches Piper voice models.
