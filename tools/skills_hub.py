#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Kopi Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Xing Bao Ku PTE LTD) but not activated
...
# ---------------------------------------------------------------------------
# Kopi centralized index source
# ---------------------------------------------------------------------------

KOPI_INDEX_URL = "https://kopi-agent.happysocial.xyz/docs/api/skills-index.json"
KOPI_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "kopi-index.json"
...
def _load_kopi_index() -> Optional[dict]:
    ...
    try:
        resp = httpx.get(KOPI_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Kopi index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Kopi index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class KopiIndexSource(SkillSource):
    """Skill source backed by the centralized Kopi Skills Index.
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        KopiIndexSource(auth=auth), # Centralized index (search + resolved install paths)
