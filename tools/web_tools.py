...
When available, Kopi can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "happysocial.xyz" or host.endswith(".happysocial.xyz")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=kopi-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "kopi-agent"
    return _exa_client
