...
Lets the LLM write a Python script that calls Kopi tools via RPC,
...
# available on every platform Kopi itself runs on.
...
"""Auto-generated Kopi tools RPC stubs."""
...
"""Auto-generated Kopi tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # Ensure the kopi-agent root is importable in the sandbox so
        ...
        # KOPI_TIMEZONE is an internal Kopi setting and must not leak
        ...
        # {KOPI_HOME}/home/ when that directory exists.
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (kopi-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and kopi-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Kopi tools programmatically. "
