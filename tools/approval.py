...
def _is_gateway_approval_context() -> bool:
    ...
    """
    if os.getenv("KOPI_CRON_SESSION"):
        return False
    if os.getenv("KOPI_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $KOPI_HOME.
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bkopi\s+gateway\s+(stop|restart)\b', "stop/restart kopi gateway (kills running agents)"),
    (r'\bkopi\s+update\b', "kopi update (restarts gateway, kills running agents)"),
    ...
    (r'\b(pkill|killall)\b.*\b(kopi|gateway|cli\.py)\b', "kill kopi/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill kopi` but not
    # `kill -9 $(pgrep -f kopi)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("KOPI_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("KOPI_INTERACTIVE")
    ...
    if is_gateway or os.getenv("KOPI_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("KOPI_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("KOPI_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("KOPI_EXEC_ASK")
