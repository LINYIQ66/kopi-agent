#!/usr/bin/env python3
"""
KOPI O Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the KOPI O Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills kopi-agent-dev,github-auth
...
try:
    from kopi_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
def load_cli_config() -> Dict[str, Any]:
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Kopi, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Kopi - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and KopiCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `kopi model`).
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && kopi` to control it.
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``KopiCLI._busy_command()``.
        """
        yield self

# ASCII Art - KOPI-AGENT logo (full width, single line - requires ~95 char terminal)
...
# ASCII Art - Kopi Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "KOPI O Agent") if _skin else "KOPI O Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Xing Bao Ku PTE LTD[/]\n"
...
class KopiCLI:
    """
    Interactive CLI for the KOPI O Agent.
    ...
    def __init__(
        ...
        # it's shared across all Kopi processes for this KOPI_HOME.
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Kopi needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous Kopi LLM (not agentic)
        ...
        if is_nous_kopi_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Xing Bao Ku PTE LTD Kopi 3 & 4 models are NOT agentic and are not "
                "designed for use with KOPI O Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Kopi: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Kopi: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Kopi config/state.
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Kopi!{_RST}")
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from kopi_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_kopi_home()}/plugins/ to get started.")
                else:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Kopi keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Kopi is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Kopi is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Kopi is busy."
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  kopi --resume {self.session_id}")
            if session_title:
                print(f"  kopi -c \"{session_title}\"")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to KOPI O Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to KOPI O Agent! Type your message or /help for commands."
        ...
        # after an OpenClaw→Kopi migration (especially migrations done by
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Kopi binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from kopi_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "KOPI O Agent")
        ...
        # Dynamic prompt: shows Kopi symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Kopi needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Kopi needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Kopi needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = KopiCLI._compute_model_picker_viewport(
...
def main(
    ...
    """
    KOPI O Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting KOPI Gateway (messaging platforms)...")
    ...
    # mode registers its own inside KopiCLI.run(), but `-q` runs
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Kopi" box, no tool-gen
