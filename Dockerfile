...
# that would otherwise accumulate when kopi runs as PID 1. See #15012.
RUN apt-get update && \
...
# that permanently disagrees with the root lock on the @kopi/ink entry.
...
# ---------- Permissions ----------
# Make install dir world-readable so any KOPI_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the kopi user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# kopi_cli/main.py succeeds (see #18800). /opt/kopi/web is build-time
# only (KOPI_WEB_DIST points at kopi_cli/web_dist) and is intentionally
# not chowned here.
USER root
RUN chmod -R a+rX /opt/kopi && \
    chown -R kopi:kopi /opt/kopi/ui-tui /opt/kopi/node_modules
# Start as root so the entrypoint can usermod/groupmod + gosu.
# If KOPI_UID is unset, the entrypoint drops to the default kopi user (10000).

# ---------- Link kopi-agent itself (editable) ----------
...
VOLUME [ "/opt/data" ]
ENTRYPOINT [ "/usr/bin/tini", "-g", "--", "/opt/kopi/docker/entrypoint.sh" ]
