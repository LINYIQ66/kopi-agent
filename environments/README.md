...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  KopiAgentBaseEnv    │  kopi_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     KopiSweEnv    TerminalBench2EvalEnv
...
**KopiAgentBaseEnv** (`kopi_base_env.py`) extends BaseEnv with kopi-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, ssh, singularity, modal, daytona, vercel_sandbox)
- Resolves kopi-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `KopiAgentBaseEnv` and implement:
...
`KopiAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as kopi-agent's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** kopi-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any kopi-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some kopi-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `kopi` -- Kopi/ChatML `<tool_call>` XML format
...
├── kopi_base_env.py            # Abstract base (KopiAgentBaseEnv)
...
│
├── kopi_swe_env/               # SWE-bench style training environment
│   └── kopi_swe_env.py
│
...
2. Create your env file inheriting from `KopiAgentBaseEnv`
...
2. Inherit from `KopiAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which kopi toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `kopi` |
