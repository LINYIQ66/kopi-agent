...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from KOPI O Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from KOPI O Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from KOPI O Agent",
    },
    {
...
class TerminalTestEnv(KopiAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/kopi-agent/.env.
        """
        env_config = TerminalTestEnvConfig(
            ...
            tokenizer_name="XingBaoKu/q-30b-t-h45-e1",
