...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs KopiAgentLoop with terminal tool only
...
Requires: pip install kopi-agent[yc-bench]
"""
...
class YCBenchEvalConfig(KopiAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends KopiAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(KopiAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The KopiAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs KopiAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our KopiAgentLoop.
