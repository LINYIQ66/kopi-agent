"""
KopiAgentBaseEnv -- Abstract Base Environment for Kopi-Agent + Atropos

Provides the Atropos integration plumbing that all kopi-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via KopiAgentLoop
...
# Ensure the kopi-agent repo root is on sys.path so that imports like
...
# Load API keys from kopi-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, KopiAgentLoop
...
# Import kopi-agent toolset infrastructure
...
class KopiAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for kopi-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of kopi toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: kopi, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/kopi-results/ "
...
class KopiAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for kopi-agent Atropos integration.
    ...
    name: Optional[str] = "kopi-agent"
    env_config_cls = KopiAgentEnvConfig

    def __init__(
        self,
        config: KopiAgentEnvConfig,
        ...
        # Set terminal environment variables so kopi tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY kopi-agent tool (terminal, file, web,
