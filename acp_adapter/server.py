"""ACP agent server — exposes KOPI O Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Kopi running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Kopi cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Kopi/OpenAI-compatible user content payload."""
...
class KopiACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Kopi AIAgent."""
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Kopi estimates ``used`` from the same
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Kopi using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="kopi-agent", version=KOPI_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Kopi' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications; merely restoring server-side state makes Kopi remember
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Kopi on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Kopi' local kawaii waiting/status
        ...
        # not the executor's. Also set KOPI_INTERACTIVE so approval.py
        ...
        def _run_agent() -> dict:
            ...
            previous_interactive = os.environ.get("KOPI_INTERACTIVE")
            ...
            try:
                ...
                # Restore KOPI_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("KOPI_INTERACTIVE", None)
                else:
                    os.environ["KOPI_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"KOPI O Agent v{KOPI_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Kopi has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
