...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: kopi)
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Kopi can degrade gracefully instead of repeatedly trying to start
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Kopi config."""
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Kopi profile name (from agent_identity)
      {workspace} — Kopi workspace name (from agent_workspace)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between KOPI O Agent and the User"
    ...
    def post_setup(self, kopi_home: str, config: dict) -> None:
        ...
        from kopi_cli.memory_setup import _curses_select
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between KOPI O Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between KOPI O Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Kopi created/used that client on the shared async loop,
