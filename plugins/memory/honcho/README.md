...
```bash
kopi honcho setup    # full interactive wizard (cloud or local)
kopi memory setup    # generic picker, also works
```
...
Host key is derived from the active Kopi profile: `kopi` (default) or `kopi.<profile>`.
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-kopi-agent`.
...
Host key is derived from the active Kopi profile: `kopi` (default) or `kopi.<profile>` (e.g. `kopi -p coder` → host key `kopi.coder`).
...
| Command | Description |
|---------|-------------|
| `kopi honcho setup` | Full interactive setup wizard |
