...
Resolution order for config file:
  1. $KOPI_HOME/honcho.json  (instance-local, enables isolated Kopi instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Kopi profile.
...
def resolve_config_path() -> Path:
    ...
    default_path = Path.home() / ".kopi" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Kopi system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Kopi profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Kopi session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Kopi session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Kopi session_id (new Honcho session each run)
