...
def cmd_sync(args) -> None:
    ...
    Scans all Kopi profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Kopi profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Kopi persistent cross-session memory.")
...
def _active_profile_name() -> str:
    """Return the active Kopi profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Kopi' identity in Honcho. Seed with 'kopi honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Kopi asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Kopi + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Kopi\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Kopi persistent memory")
    ...
    if user_files:
        ...
        print("  (Kopi calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Kopi, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Kopi:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Kopi:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Kopi:   Honcho observes every message and updates representations")
    ...
    print("    Kopi:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Kopi Honcho",
    )
