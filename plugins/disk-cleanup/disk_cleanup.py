"""disk_cleanup — ephemeral file cleanup for KOPI O Agent.
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under KOPI_HOME (but leave KOPI_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "kopi-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(kopi_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == kopi_home:
                continue
            try:
                rel_parts = dirpath.relative_to(kopi_home).parts
            except ValueError:
                continue
