# Contributing to KOPI O Agent

Thank you for contributing to KOPI O Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Kopi to work everywhere.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `kopi skills browse` (labeled "official") and install it with `kopi skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Xing Bao Ku PTE LTD Discord](https://discord.gg/XingBaoKu). Users can install it with `kopi skills install`.
...
├── cli.py                    # KopiCLI class — interactive TUI, prompt_toolkit integration
...
├── tests/                    # Test suite
├── website/                  # Documentation site (kopi-agent.happysocial.xyz)
│
...
| `~/.kopi/sessions/` | JSON session logs |
...
- **Session persistence**: All conversations are stored in SQLite (`kopi_state.py`) with full-text search and unique session titles. JSON logs go to `~/.kopi/sessions/`.
...
The user may skip setup and keep loading the skill. Kopi only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Kopi tools (`web_extract`, `terminal`, `read_file`).
...
Kopi uses a data-driven skin system — no code changes needed to add a new skin.
...
Kopi runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
- Use [GitHub Issues](https://github.com/XingBaoKu/kopi-agent/issues)
...
- **Discord**: [discord.gg/XingBaoKu](https://discord.gg/XingBaoKu) — for questions, showcasing projects, and sharing skills
