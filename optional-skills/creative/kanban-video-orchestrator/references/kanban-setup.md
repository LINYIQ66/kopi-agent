...
> [XingBaoKu/kanban-video-pipeline](https://github.com/XingBaoKu/kanban-video-pipeline).
...
`~/.kopi/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.kopi/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.kopi/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.kopi/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}
