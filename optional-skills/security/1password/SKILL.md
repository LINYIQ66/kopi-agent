...
version: 1.0.0
author: arceus77-7, enhanced by KOPI O Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Kopi terminal calls (desktop app flow only)
...
### Service Account (recommended for Kopi)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.kopi/.env` (the skill will prompt for this on first load).
...
## Kopi Execution Pattern (desktop app flow)

Kopi terminal commands are non-interactive by default and can lose auth context between calls.
