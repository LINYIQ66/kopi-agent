...
- smoke-test a server with the FastMCP CLI before wiring it into Kopi or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Kopi. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Kopi integration, either:
...
### Kopi cannot see the deployed server

The server-building part may be correct while the Kopi config is not. Load the `native-mcp` skill and configure the server in `~/.kopi/config.yaml`, then restart Kopi.
