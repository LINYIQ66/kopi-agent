...
Diagnoses issues with KOPI O Agent setup.
"""
...
from kopi_cli.vercel_auth import describe_vercel_auth
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Kopi Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Kopi' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `kopi auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Kopi a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# KOPI O Agent Persona\n\n"
                "<!-- Edit this file to customize how Kopi communicates. -->\n\n"
                "You are Kopi, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    # =========================================================================
    # Check: Command installation (kopi bin symlink)
    # =========================================================================
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'kopi-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'kopi-agent[vercel]'")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                check_fail("Honcho API key or base URL not set", "run: kopi memory setup")
                issues.append("No Honcho API key — run 'kopi memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run kopi memory setup)")
