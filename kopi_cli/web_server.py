...
app = FastAPI(title="KOPI O Agent", version=__version__)
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``kopi dashboard --tui``
# or KOPI_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
def _spawn_kopi_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "kopi_cli.main", *subcommand]
...
async def restart_gateway():
    ...
    try:
        proc = _spawn_kopi_action(["gateway", "restart"], "gateway-restart")
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Kopi resolves Anthropic creds in this order at runtime:
    1. ``~/.kopi/.anthropic_oauth.json`` — Kopi-managed PKCE flow
    ...
    try:
        ...
        _KOPI_OAUTH_FILE = None  # type: ignore
    ...
    if kopi_creds and kopi_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "kopi_pkce",
            "source_label": f"Kopi PKCE ({_KOPI_OAUTH_FILE})",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://portal.happysocial.xyz",
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Kopi-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _KOPI_OAUTH_FILE
            if _KOPI_OAUTH_FILE.exists():
                _KOPI_OAUTH_FILE.unlink()
        except Exception:
            pass
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _KOPI_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _KOPI_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from kopi_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from kopi_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/kopi"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_script = (
            ...
            f'window.__KOPI_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
...
def _discover_dashboard_plugins() -> list:
    ...
    if os.environ.get("KOPI_ENABLE_PROJECT_PLUGINS"):
...
def start_server(
    ...
    *,
    ...
    print(f"  Kopi Web UI → http://{host}:{port}")
