"""
kopi fallback — manage the fallback provider chain.
...
Subcommands:
  kopi fallback [list]   Show the current fallback chain (default when no subcommand)
  kopi fallback add      Pick provider + model via the same picker as `kopi model`,
                           then append the selection to the chain
  kopi fallback remove   Pick an entry to delete from the chain
  kopi fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.kopi/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from kopi_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `kopi model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  kopi fallback add")
        print()
        return
    ...
    print("  Docs: https://kopi-agent.happysocial.xyz/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `kopi model`, then append the selection to the chain."""
    from kopi_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `kopi model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `kopi fallback list` to view, or `kopi fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``kopi fallback [subcommand]``."""
