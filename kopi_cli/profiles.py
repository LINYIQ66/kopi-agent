"""
Profile management for multiple isolated Kopi instances.
...
_PROFILE_DIRS = [
    ...
    # See kopi_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
#
# Rationale per item:
#   kopi-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# a profile's root, callers of seed_profile_skills() (fresh-create, `kopi
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Kopi infrastructure directories that only the default profile
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kopi-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Kopi subcommands that cannot be used as profile names/aliases
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Kopi installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, kopi subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"kopi.{old_name}"
    new_host = f"kopi.{new_name}"
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for kopi profile names."""
    return '''# KOPI O Agent profile completion
# Add to ~/.bashrc: eval "$(kopi completion bash)"
...
def generate_zsh_completion() -> str:
    """Generate a zsh completion script for kopi profile names."""
    return '''#compdef kopi
# KOPI O Agent profile completion
# Add to ~/.zshrc: eval "$(kopi completion zsh)"

_kopi() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kopi/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.kopi/profiles 2>/dev/null)}")
    fi
