"""
Unified tool configuration for KOPI O Agent.
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. KOPI_CUA_DRIVER_VERSION is an
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `kopi update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Kopi process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kopi_home()}/kopi-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in kopi-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/xingbaoku/kopi-agent:latest"
            )
            return
        ...
        _print_info("    Restart Kopi for tracing to take effect.")
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[kopi-cli, spotify]`` after enabling Spotify via ``kopi
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Kopi Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://kopi-agent.happysocial.xyz/docs/user-guide/features/tools", Colors.DIM))
    print()
