...
Usage:
    ...
    kopi honcho migrate                  # Step-by-step migration guide: OpenClaw native → Kopi + Honcho
    kopi version             Show version
    kopi update              Update to latest version
    kopi uninstall           Uninstall KOPI O Agent
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set KOPI_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
...
try:
    from kopi_cli.config import load_config as _load_config_early
...
from kopi_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Kopi itself has been explicitly configured (model
    ...
    # Only count these if Kopi has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Kopi to use their tokens.
    if _has_kopi_config:
        try:
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("KOPI_TUI") == "1"
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Kopi isn't configured yet -- no API keys or providers found."
        )
        print()
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Kopi?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ KOPI O Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  kopi gateway")
...
#
# Kopi uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Kopi only falls back to a lightweight backend (OpenRouter,")
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from kopi_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  Kopi will probe your")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Kopi turns to `copilot --acp`.")
    print("  Kopi currently starts its own ACP subprocess for each request.")
    print("  Kopi uses your selected model as a hint for the Copilot ACP session.")
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Kopi typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Kopi, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Kopi will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Kopi CLI with a provider."""
...
def cmd_backup(args):
    """Back up Kopi home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Kopi backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"KOPI O Agent v{__version__} ({__release_date__})")
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``kopi_cli/web_dist/`` (per vite.config.ts
    outDir: "../kopi_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "kopi_cli" / "web_dist"
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://kopi-agent.happysocial.xyz/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    """Update KOPI O Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/XingBaoKu/kopi-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="kopi-update-")
        zip_path = os.path.join(tmp_dir, f"kopi-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to kopi-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"kopi-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Kopi behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Kopi couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Kopi couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Kopi behaves unexpectedly.")
    return True
...
OFFICIAL_REPO_URLS = {
    "https://github.com/XingBaoKu/kopi-agent.git",
    "git@github.com:XingBaoKu/kopi-agent.git",
    "https://github.com/XingBaoKu/kopi-agent",
    "git@github.com:XingBaoKu/kopi-agent",
}
OFFICIAL_REPO_URL = "https://github.com/XingBaoKu/kopi-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from XingBaoKu/kopi-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/XingBaoKu/kopi-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/XingBaoKu/kopi-agent.git' to add later."
            )
...
def _kopi_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "kopi.exe",
        scripts_dir / "kopi-gateway.exe",
    ]


def _quarantine_running_kopi_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
...
def _ensure_fhs_path_guard() -> None:
    ...
    where ``bash -i -c 'command -v kopi'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/kopi, code at /usr/local/lib/kopi-agent).
    ...
    path_comment = (
        "# KOPI O Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update KOPI O Agent to the latest version.
    ...
    if is_managed():
        managed_error("update KOPI O Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Tying the refresh to ``kopi update`` gives users a predictable
        ...
        # Warn if legacy Kopi gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_kopi_units():
                print()
                print("⚠ Legacy Kopi gateway unit(s) detected:")
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → kopi -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.kopi)")
            else:
        ...
        if data.get("kopi_requires"):
            print(f"Requires:     Kopi {data['kopi_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.kopi_requires:
        print(f"  Requires: Kopi {mf.kopi_requires}")
...
def cmd_dashboard(args):
    ...
    if "KOPI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
    ...
    embedded_chat = args.tui or os.environ.get("KOPI_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_logs(args):
    """View and filter Kopi log files."""
...
def main():
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://kopi-agent.happysocial.xyz/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure KOPI O Agent with an interactive wizard. "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Kopi.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Kopi")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Kopi CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Kopi with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of KOPI O Agent components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with KOPI O Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Kopi setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for KOPI O Agent. Use 'kopi debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kopi debug share              Upload debug report and print URL
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Kopi home directory to a zip file",
        description="Create a zip archive of your entire Kopi configuration, "
        "skills, sessions, and data (excludes the kopi-agent codebase). "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Kopi backup from a zip file",
        description="Extract a previously created Kopi backup into your "
        "Kopi home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage KOPI O Agent configuration",
    )
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Kopi as an MCP server",
        description=(
            "Manage MCP server connections and run Kopi as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'kopi mcp add' to connect to a new server, or\n"
            "'kopi mcp serve' to expose Kopi conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Kopi as an MCP server (expose conversations to other agents)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Kopi",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Kopi",
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update KOPI O Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall KOPI O Agent",
        description="Remove KOPI O Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run KOPI O Agent as an ACP (Agent Client Protocol) server",
        description="Start KOPI O Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch KOPI O Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Kopi instances",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Kopi profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the KOPI O Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `kopi --tui` via PTY/WebSocket). "
            "Alternatively set KOPI_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `kopi dashboard` cmdlines and SIGTERM them directly — the same
    # path `kopi update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running kopi dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running kopi dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Kopi log files",
