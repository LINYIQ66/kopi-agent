...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Kopi refreshes the cache from
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.happysocial.xyz").rstrip("/")
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Kopi that doesn't ship that model in its hardcoded curated
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from kopi_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.happysocial.xyz"
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Xing Bao Ku PTE LTD subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``kopi model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``kopi auth add openai-codex`` without
    ``kopi model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    kopi-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — kopi-agent
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from kopi_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return kopi-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; kopi's picker expects
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.happysocial.xyz"
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.happysocial.xyz/v1
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Kopi' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Kopi should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Kopi release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Kopi release). Falls back to the hardcoded catalog if no token
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Kopi' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Kopi will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Kopi cannot verify the model name."
