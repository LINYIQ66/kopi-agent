"""``kopi debug`` debug tools for KOPI O Agent.
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Kopi version, provider, which API keys
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "kopi-agent/debug-share"},
    )
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"kopi debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "kopi-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----KopiDebugBoundary9f3c"
...
def run_debug_share(args):
    ...
    print(f"To delete now:  kopi debug delete <url>")

    print(f"\nShare these links with the Kopi team for support.")
