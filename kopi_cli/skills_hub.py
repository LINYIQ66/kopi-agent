...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Xing Bao Ku PTE LTD[/]")
    c.print()
    ...
    c.print("[dim]Tip: 'kopi skills search <query>' searches deeper across all registries[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by Xing Bao Ku PTE LTD.[/]\n\n"
                "It ships with kopi-agent but is not activated by default.\n"
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print("Usage: kopi skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n")
