...
def remove_path_from_shell_configs():
    """Remove Kopi PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing kopi-agent or kopi PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# KOPI O Agent" comment and following line
                if '# KOPI O Agent' in line or '# kopi-agent' in line:
                    skip_next = True
                    continue
                ...
                # Remove any PATH line containing kopi
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'kopi_cli' in content or 'kopi-agent' in content:
                    wrapper.unlink()
...
def _kopi_path_markers(kopi_home: Path) -> list[str]:
    """Path-entry substrings that identify Kopi-owned User-PATH entries."""
    root = str(kopi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare kopi-agent install dir.
    markers = [root + "\\kopi-agent", root + "\\git", root + "\\node", root + "\\venv"]
...
def remove_path_from_windows_registry(kopi_home: Path) -> list[str]:
    """Strip Kopi-owned entries from User-scope PATH in the registry.
...
def run_uninstall(args):
    ...
    print(color("│            ⚕ KOPI O Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Kopi data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Kopi code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # like C:\Users\<u>\AppData\Local\kopi\git\cmd, not %LOCALAPPDATA%.
        ...
        if removed_path_entries:
            ...
            log_info("No Kopi-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Kopi-set User env vars to remove")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.kopi/, just remove the kopi-agent subdir
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/XingBaoKu/kopi-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/XingBaoKu/kopi-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using KOPI O Agent! ⚕")
    print()
