...
``<root>`` is the **shared Kopi root** (the parent of any active
...
the profile's Kopi home but are otherwise isolated: a worker spawned
...
# enough that kebab-case names like ``atm10-server`` or ``kopi-agent``
...
def kanban_home() -> Path:
    """Return the shared Kopi root that anchors the kanban board.
...
def create_task(
    ...
    *,
    ...
    each name to ``kopi --skills ...`` alongside the built-in
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Kopi
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Kopi profile.
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Kopi profile.
        # `_default_spawn` invokes ``kopi -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Kopi
...
def _resolve_kopi_argv() -> list[str]:
    ...
    1. ``shutil.which("kopi")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m kopi_cli.main`` — fallback for setups where
       Kopi is launched from a venv and the ``kopi`` shim is not on
    ...
    if kopi_bin:
        return [kopi_bin]
    # Fallback to the module form. ``kopi_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``kopi`` package — there is no ``kopi`` package to import.
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install KOPI O Agent or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Kopi root exists
