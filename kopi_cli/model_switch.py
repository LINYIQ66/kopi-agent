...
_KOPI_MODEL_WARNING = (
    "Xing Bao Ku PTE LTD Kopi 3 & 4 models are NOT agentic and are not designed "
    "for use with KOPI O Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Xing Bao Ku PTE LTD Kopi 3 / Kopi 4 chat families.
...
#   XingBaoKu/Kopi-3-Llama-3.1-70B, kopi-4-405b, openrouter/kopi3:70b
...
def switch_model(
    ...
    # Mirrors the same logic in kopi_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    # https://kopi-agent.happysocial.xyz/docs/api/model-catalog.json so
    ...
    for kopi_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                ...
                if store and kopi_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Kopi release.
    ...
    for pid, overlay in KOPI_OVERLAYS.items():
        ...
        if kopi_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(kopi_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(kopi_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(kopi_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Kopi slug, fall back to overlay key
            model_ids = curated.get(kopi_slug, []) or curated.get(pid, [])
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Kopi writes ``models:`` as a dict keyed by model id
            # (see kopi_cli/main.py::_save_custom_provider); older
