...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m kopi_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Kopi_Gateway"
_TASK_DESCRIPTION = "KOPI O Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``Kopi_Gateway``
    Named profile X: ``Kopi_Gateway_<X>``
    """
    _assert_windows()
...
def get_task_script_path() -> Path:
    ...
    Kopi installs stay self-contained).
    """
    _assert_windows()
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m kopi_cli.main [--profile X] gateway run --replace``
    ...
    prog_args = [python_path, "-m", "kopi_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m kopi_cli.main gateway run --replace``
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from kopi_cli.config import get_kopi_home
        print(f"    type {Path(get_kopi_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_kopi_home()).resolve()}\\logs\\gateway-stdio.log")
...
def stop() -> None:
    ...
    _assert_windows()
    from kopi_cli.gateway import kill_gateway_processes
