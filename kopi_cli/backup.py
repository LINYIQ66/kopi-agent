...
(excluding the kopi-agent repo and transient files).
...
_EXCLUDED_DIRS = {
    "kopi-agent",     # the codebase repo — re-clone instead
...
def run_backup(args) -> None:
    """Create a zip backup of the Kopi home directory."""
    kopi_root = get_default_kopi_root()

    if not kopi_root.is_dir():
        print(f"Error: Kopi home directory not found at {kopi_root}")
        sys.exit(1)
    ...
    for dirpath, dirnames, filenames in os.walk(kopi_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside kopi root
            try:
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Kopi backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a Kopi backup "
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Kopi configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: kopi profile list  (after installing kopi)")

        # Guidance
        print()
        if not (kopi_root / "kopi-agent").is_dir():
            print("Note: The kopi-agent codebase was not included in the backup.")
        ...
        print("Done. Your Kopi configuration has been restored.")
