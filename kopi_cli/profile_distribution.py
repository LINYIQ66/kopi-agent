"""Profile distributions — shareable, packaged Kopi profiles via git.

A distribution is a Kopi profile published as a git repository (or
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "kopi-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def check_kopi_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Kopi {op}{target}, "
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Kopi profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Kopi distribution."
        )
