        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "kopi-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "kopi_cli.main --profile",
        "kopi_cli.main -p",
        "kopi_cli/main.py gateway",
        "kopi_cli/main.py --profile",
        "kopi_cli/main.py -p",
        "kopi gateway",
        "gateway/run.py",
    ]
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"KOPI_HOME={current_home}" in command
            )
        ...
        if "--profile " in command or " -p " in command:
            return False
        if "KOPI_HOME=" in command and f"KOPI_HOME={current_home}" not in command:
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Kopi profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}kopi gateway restart{scope_flag}")
...
SERVICE_DESCRIPTION = "KOPI O Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(kopi_home: str | None = None) -> str:
...
# Legacy service names from older Kopi installs that predate the
...
def _find_legacy_kopi_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Kopi gateway units.

    Detects unit files installed by older Kopi versions that used a
...
def has_legacy_kopi_units() -> bool:
    """Return True when any legacy Kopi gateway unit files exist."""
    return bool(_find_legacy_kopi_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Kopi gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Kopi gateway unit(s) detected from an older install:")
...
def remove_legacy_kopi_units(
    ...
    legacy = _find_legacy_kopi_units()
    if not legacy:
        print("No legacy Kopi gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Kopi gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Kopi can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo kopi gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo kopi gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo kopi gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Kopi often sets ``HOME`` to a profile-scoped directory, but
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.kopi/kopi-agent  -> /home/alice/.kopi/kopi-agent
...
User={username}
Group={group_name}
ExecStart={python_path} -m kopi_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Kopi install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}kopi gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}kopi gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}kopi gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}kopi gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}kopi gateway start{scope_flag}")
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Kopi install")
    return True
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Kopi install")
    else:
        print("⚠ Service definition is stale relative to the current Kopi install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Kopi gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'kopi' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/kopi/docker/entrypoint.sh before the Kopi command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$KOPI_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set KOPI_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Kopi will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @kopi:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. kopi) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Kopi delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Kopi agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Kopi will use (e.g., kopi@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Kopi connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Kopi will connect automatically over WebSocket",
        ],
        "vars": [
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Kopi will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Kopi will store the returned account_id/token in ~/.kopi/.env.")
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `kopi pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"KopiAgent\"")
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo kopi gateway install --system")
                ...
                print_info("  For persistence:   tmux new -s kopi 'kopi gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from kopi_constants import display_kopi_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: kopi gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup kopi gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info("  Consider running in foreground instead: kopi gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s kopi 'kopi gateway run'")
                print()
            ...
            print()
            print("  kopi gateway run                              # direct foreground")
            print("  tmux new -s kopi 'kopi gateway run'         # persistent via tmux")
            print("  nohup kopi gateway run > ~/.kopi/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup kopi gateway run > ~/.kopi/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s kopi 'kopi gateway run'         # persistent via tmux")
                    print("  nohup kopi gateway run > ~/.kopi/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  kopi gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  kopi gateway install  # Install as user service")
                    print("  sudo kopi gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Kopi gateway unit files from
