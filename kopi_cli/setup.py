"""
Interactive setup wizard for KOPI O Agent.
...
_DOCS_BASE = "https://kopi-agent.happysocial.xyz/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Kopi using environment variables or config commands:")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Kopi can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Kopi can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'kopi setup' or configure AUXILIARY_VISION_* settings")
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"kopi setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Kopi runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'kopi-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'kopi-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://kopi-agent.happysocial.xyz/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Kopi",
            bot_description="Your Kopi agent on Slack",
        )
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Kopi delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Kopi to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://kopi-agent.happysocial.xyz/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://kopi-agent.happysocial.xyz/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  kopi config edit")
    print_info("   Open config in your editor:  kopi config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Kopi from anywhere.")
...
# Config values may have different semantics between OpenClaw and Kopi.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Kopi to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Kopi at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Kopi at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Kopi at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Kopi at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Kopi equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Kopi config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Kopi.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Kopi's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(kopi_home: Path) -> bool:
    ...
    print_info("Kopi can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Kopi configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Kopi config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Kopi (use kopi claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             ⚕ KOPI O Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your KOPI O Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Kopi configured.")
        ...
        setup_mode = prompt_choice("How would you like to set up Kopi?", [
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch kopi chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, kopi_home, is_existing: bool):
    ...
    the user can customize later via ``kopi setup <section>``.
    """
...
def _run_quick_setup(config: dict, kopi_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Kopi to messaging apps to chat from anywhere.")
