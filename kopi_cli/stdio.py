...
on Japanese locales, etc.).  Kopi's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Kopi is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Kopi-managed Git install directories so
    ...
    # already-running shells don't see the change — which means kopi
    ...
    # install without Kopi-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Kopi's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Kopi's
    ...
    Set this before launching Kopi (User env var in Windows Settings, or
    export in a PowerShell profile) and Kopi picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Kopi-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Kopi spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Kopi-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Kopi venv Scripts directory — host of the kopi.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes kopi, but harmless to include.
        os.path.join(local_appdata, "kopi", "kopi-agent", "venv", "Scripts"),
