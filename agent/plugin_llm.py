...
Plugins built on KOPI O Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Kopi
...
embedded slugs, no shorthands. This mirrors Kopi' main config
...
handles every provider, fallback chain, and per-task override Kopi
supports.
"""
