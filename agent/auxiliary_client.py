...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Kopi triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://kopi-agent.happysocial.xyz",
    "X-Title": "KOPI O Agent",
...
NOUS_EXTRA_BODY = {"tags": ["product=kopi-agent", "client=aux"]}
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.happysocial.xyz/v1"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (KOPI O Agent)",
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Kopi auth store.
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second kopi
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.happysocial.xyz"):
        return "nous"
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.happysocial.xyz")
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.happysocial.xyz")
        )
        if _is_auth_error(first_err) and client_is_nous:
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "nous",
