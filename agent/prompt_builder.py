...
KOPI_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using KOPI O Agent "
    "itself, load the `kopi-agent` skill with skill_view(name='kopi-agent') "
    "before answering. Docs: https://kopi-agent.happysocial.xyz/docs"
)
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot KOPI O Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `kopi-agent` skill "
...
def _load_kopi_md(cwd_path: Path) -> str:
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".kopi.md")
    except Exception as e:
