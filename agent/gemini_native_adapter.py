...
Kopi keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Kopi's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Kopi)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Kopi typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Kopi retry/error classification."""
...
class GeminiNativeClient:
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "kopi-agent (gemini-native)",
        }
