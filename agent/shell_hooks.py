...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Kopi-canonical
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Kopi wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Kopi
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Kopi is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical Kopi-wire-shape response."""
