"""
Kopi MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Kopi-specific extra)
...
class EventBridge:
    ...
    This is the Kopi equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Kopi MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "kopi",
        instructions=(
            "KOPI O Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Kopi MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
