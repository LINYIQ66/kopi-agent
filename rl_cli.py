...
if tinker_atropos_dir.exists():
    ...
    os.environ['KOPI_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to kopi-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.kopi/config.yaml if not provided)
    ...
    """
    # Load config from ~/.kopi/config.yaml
    config = load_kopi_config()
