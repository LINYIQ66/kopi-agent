...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("KOPI_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent KopiCLI subprocess for slash commands."""
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from kopi_cli.model_switch import parse_model_flags, switch_model
    ...
    os.environ["KOPI_MODEL"] = result.new_model
    os.environ["KOPI_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # KOPI_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["KOPI_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["KOPI_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"kopi_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `kopi model` and
        ...
        from kopi_cli.auth import PROVIDER_REGISTRY as _auth_reg
        ...
        for entry in CANONICAL_PROVIDERS:
            if entry.slug in authed_map:
                ...
                if auth_type == "api_key" and key_env:
                    ...
                    warning = f"run `kopi model` to configure ({auth_type})"
                ordered.append(
                    {
                        "slug": entry.slug,
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from kopi_cli.browser_connect import try_launch_chrome_debug
