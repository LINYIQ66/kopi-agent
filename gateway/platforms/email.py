...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "kopi-agent" "version" "{_kopi_version}" '
            '"vendor" "XingBaoKu" '
            '"support-email" "noreply@happysocial.xyz")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "KOPI O Agent")
        ...
        msg_id = f"<kopi-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
