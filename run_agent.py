...
class _SafeWriter:
    ...
    When kopi-agent runs as a systemd service, Docker container, or headless
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"KopiAgent/{_KOPI_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[kopi-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        setup_logging(kopi_home=_kopi_home)
        ...
        os.environ["KOPI_SESSION_ID"] = self.session_id
        try:
        ...
        self.logs_dir = kopi_home / "sessions"
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by KOPI O Agent.  Choose a model with at least "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("KOPI_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Kopi' minimum context.
        """
        ...
        try:
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Kopi "
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("KOPI_API_TIMEOUT", 1800.0))
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("KOPI_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        # Pointer to the kopi-agent skill + docs for user questions about Kopi itself.
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the kopi-agent install
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("KOPI_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("KOPI_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            _base_timeout = (
                ...
                else float(os.getenv("KOPI_API_TIMEOUT", 1800.0))
            )
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("KOPI_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("KOPI_STREAM_STALE_TIMEOUT", 180.0))
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "kopi",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        """
        if base_url_host_matches(self._base_url_lower, "happysocial.xyz"):
            return True
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        # compression (not a fresh /new). Plugin engines (e.g. kopi-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See kopi-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=kopi-agent"]
    ...
    def run_conversation(
        ...
        # ``kopi logs --session <id>`` can filter a single conversation.
        ...
        # are reused.  The system prompt is Kopi's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Kopi internals.
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: kopi login --provider nous")
                        print(f"{self.log_prefix}     • Check credits / billing: https://portal.happysocial.xyz")
                    ...
                    if (
                        ...
                        from kopi_constants import display_kopi_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Kopi-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: kopi config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: kopi config set ANTHROPIC_API_KEY \"\"")
                    ...
                    # providers (DeepSeek, Kimi, MiMo, Kopi).  A 429 can
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `kopi auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: kopi setup", force=True)
