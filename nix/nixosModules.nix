# nix/nixosModules.nix — NixOS module for kopi-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.kopi-agent;
    effectivePackage =
    ...
    kopi-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-kopi-agent)
  ...
  in {
    options.services.kopi-agent = with lib; {
      ...
      package = mkOption {
        type = types.package;
        default = kopi-agent;
        description = "The kopi-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .kopi/ subdir (HERMES_HOME).";
      };
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in kopi-agent's
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.kopi-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the kopi CLI to system PATH and export HERMES_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.kopi/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.HERMES_HOME = "${cfg.stateDir}/.kopi";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.kopi-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.kopi-agent: container.enable is true and container.hostUsers
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."kopi-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.kopi/config.yaml
          '' else ''
            ...
            chmod 0640 ${cfg.stateDir}/.kopi/config.yaml
          ''}
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.kopi/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    ...
    HERMES_CONTAINER_MODE_EOF
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.kopi" ]; then
                  rm -f "${symlinkPath}"
                  echo "kopi-agent: removed symlink ${symlinkPath}"
                fi
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "kopi-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          # KOPI reads $HERMES_HOME/.env at startup via load_kopi_dotenv(),
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.kopi-agent = {
          ...
          serviceConfig = {
            ...
            # $HERMES_HOME/.env by the activation script. load_kopi_dotenv()
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env HERMES_HOME=${containerDataDir}/.kopi \
