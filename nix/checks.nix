...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      kopi-agent = self'.packages.default;
      kopiVenv = kopi-agent.kopiVenv;
    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "kopi-package-contents" { } ''
          ...
          test -x ${kopi-agent}/bin/kopi || (echo "FAIL: kopi binary missing"; exit 1)
          test -x ${kopi-agent}/bin/kopi-agent || (echo "FAIL: kopi-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${kopi-agent}/bin/kopi version 2>&1 | grep -qi "kopi" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "kopi-entry-points-sync" { } ''
          ...
          for bin in kopi kopi-agent kopi-acp; do
            test -x ${kopi-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "kopi-cli-commands" { } ''
          ...
          ${kopi-agent}/bin/kopi --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${kopi-agent}/bin/kopi --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "kopi-bundled-skills" { } ''
          ...
          test -d ${kopi-agent}/share/kopi-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${kopi-agent}/share/kopi-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${kopi-agent}/bin/kopi || \
        ...
        bundled-plugins = pkgs.runCommand "kopi-bundled-plugins" { } ''
          ...
          test -d ${kopi-agent}/share/kopi-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${kopi-agent}/share/kopi-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${kopi-agent}/bin/kopi || \
        ...
        bundled-tui = pkgs.runCommand "kopi-bundled-tui" { } ''
          ...
          test -d ${kopi-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${kopi-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${kopi-agent}/bin/kopi || \
        ...
        kopi-node = pkgs.runCommand "kopi-node-version" { } ''
          ...
          grep -q "HERMES_NODE" ${kopi-agent}/bin/kopi || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${kopi-agent}/bin/kopi)
        ...
        managed-guard = pkgs.runCommand "kopi-managed-guard" { } ''
          ...
          check_blocked "config set" ${kopi-agent}/bin/kopi config set model foo
          check_blocked "config edit" ${kopi-agent}/bin/kopi config edit
        ...
        extra-python-packages = let
          ...
          kopiWithExtra = kopi-agent.override {
        ...
        in pkgs.runCommand "kopi-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${kopi-agent}/bin/kopi; then
        ...
        extra-dependency-groups = let
          kopiWithGroups = kopi-agent.override {
