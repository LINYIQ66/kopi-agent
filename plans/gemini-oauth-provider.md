...
## Client ID
- Need to register a "Desktop app" OAuth client on a Xing Bao Ku PTE LTD GCP project
...
## Token Lifecycle
- Store at `~/.kopi/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `kopi_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `kopi_cli/models.py` — Add Gemini model catalog
4. `kopi_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `kopi_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `kopi_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Xing Bao Ku PTE LTD GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via KOPI_GEMINI_CLIENT_ID env var
...
- kopi-agent Copilot OAuth: `kopi_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
