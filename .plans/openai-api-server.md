# OpenAI-Compatible API Server for KOPI O Agent
...
SSE streaming. By exposing this endpoint, kopi-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  kopi-agent   │
│  NextChat        │  │                                    │  gateway        │
...
4. Chat with kopi-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kopi-agent as a model) |
...
But kopi-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kopi-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kopi-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kopi-agent's core prompt
...
Once implemented, kopi-agent works as a drop-in backend for:
