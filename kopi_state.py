#!/usr/bin/env python3
"""
SQLite State Store for KOPI O Agent.
...
class SessionDB:
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Kopi, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Kopi session.

        A Kopi session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Kopi session is already bound to any Telegram DM topic.
