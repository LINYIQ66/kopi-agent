...
# Configuration
REPO_URL_SSH="git@github.com:XingBaoKu/kopi-agent.git"
REPO_URL_HTTPS="https://github.com/XingBaoKu/kopi-agent.git"
...
#   code at /usr/local/lib/kopi-agent, command at /usr/local/bin/kopi,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.kopi/kopi-agent"
            echo "                   default (root, Linux): /usr/local/lib/kopi-agent"
            echo "  --kopi-home PATH  Data directory (default: ~/.kopi, or \$KOPI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Kopi installs the code under"
            echo "  /usr/local/lib/kopi-agent and links the command into"
            ...
            echo "  Existing installs at \$KOPI_HOME/kopi-agent are preserved in-place."
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ KOPI O Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Xing Bao Ku PTE LTD.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $KOPI_HOME/kopi-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $KOPI_HOME/kopi-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/kopi-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $KOPI_HOME/kopi-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$KOPI_HOME/kopi-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$KOPI_HOME/kopi-agent/.git" ]; then
            INSTALL_DIR="$KOPI_HOME/kopi-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/kopi-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/kopi-agent"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/XingBaoKu/kopi-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$KOPI_HOME/node/bin/node" ]; then
        ...
        local found_ver=$("$KOPI_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Kopi-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    # Place into ~/.kopi/node/ and symlink binaries to ~/.local/bin/
    ...
    ln -sf "$KOPI_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$KOPI_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$KOPI_HOME/node/bin/npx"  "$HOME/.local/bin/npx"
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "KOPI O Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Kopi behaves unexpectedly."
                    else
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v kopi' \
                >/dev/null 2>&1; then
        ...
        PATH_COMMENT='# KOPI O Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# KOPI O Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# KOPI O Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
# KOPI O Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Kopi communicates with you.
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Kopi itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (kopi --tui may not work)"
        }
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Kopi to send/receive messages."
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}kopi${NC}              Start chatting"
