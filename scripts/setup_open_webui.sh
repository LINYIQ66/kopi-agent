...
# Bootstrap Open WebUI against KOPI O Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.kopi/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-kopi.sh
...
#   KOPI_API_PORT=8642
#   KOPI_API_HOST=127.0.0.1
#   KOPI_API_MODEL_NAME='KOPI O Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-KOPI O Agent WebUI}"
...
KOPI_ENV_FILE="${KOPI_ENV_FILE:-$HOME/.kopi/.env}"
KOPI_API_PORT="${KOPI_API_PORT:-8642}"
KOPI_API_HOST="${KOPI_API_HOST:-127.0.0.1}"
KOPI_API_CONNECT_HOST="${KOPI_API_CONNECT_HOST:-127.0.0.1}"
KOPI_API_MODEL_NAME="${KOPI_API_MODEL_NAME:-KOPI O Agent}"
KOPI_API_BASE_URL="http://${KOPI_API_CONNECT_HOST}:${KOPI_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-kopi.sh"
LOG_DIR="$HOME/.kopi/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$KOPI_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$KOPI_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.kopi.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.kopi</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.kopi"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.kopi"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-kopi.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to KOPI O Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-kopi.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.kopi/logs/openwebui.log
StandardError=append:%h/.kopi/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-kopi.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$KOPI_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Kopi API server is configured...'
  upsert_env API_SERVER_ENABLED true "$KOPI_ENV_FILE"
  upsert_env API_SERVER_HOST "$KOPI_API_HOST" "$KOPI_ENV_FILE"
  upsert_env API_SERVER_PORT "$KOPI_API_PORT" "$KOPI_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$KOPI_API_MODEL_NAME" "$KOPI_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$KOPI_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Kopi gateway so API server settings take effect...'
  kopi gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${KOPI_API_CONNECT_HOST}:${KOPI_API_PORT}/health" >/dev/null; then
    log 'Kopi API server did not answer on the first check. Trying to start gateway in the background...'
    nohup kopi gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${KOPI_API_CONNECT_HOST}:${KOPI_API_PORT}/health" >/dev/null
  ...
  log "Kopi API endpoint: ${KOPI_API_BASE_URL}"
