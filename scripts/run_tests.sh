#!/usr/bin/env bash
# Canonical test runner for kopi-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.kopi/kopi-agent/venv"; do
...
# Unset KOPI_* behavioral vars too.
unset KOPI_YOLO_MODE KOPI_INTERACTIVE KOPI_QUIET KOPI_TOOL_PROGRESS \
      KOPI_TOOL_PROGRESS_MODE KOPI_MAX_ITERATIONS KOPI_SESSION_PLATFORM \
      KOPI_SESSION_CHAT_ID KOPI_SESSION_CHAT_NAME KOPI_SESSION_THREAD_ID \
      KOPI_SESSION_SOURCE KOPI_SESSION_KEY KOPI_GATEWAY_SESSION \
      KOPI_CRON_SESSION \
      KOPI_PLATFORM KOPI_INFERENCE_PROVIDER KOPI_MANAGED KOPI_DEV \
      KOPI_CONTAINER KOPI_EPHEMERAL_SYSTEM_PROMPT KOPI_TIMEZONE \
      KOPI_REDACT_SECRETS KOPI_BACKGROUND_NOTIFICATIONS KOPI_EXEC_ASK \
      KOPI_HOME_MODE 2>/dev/null || true
...
# If a system-wide kopi pytest_live_guard plugin is installed at
# $HOME/.kopi/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.kopi/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.kopi:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.kopi" ;;
  esac
